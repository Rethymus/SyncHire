...
<p align="center">
  <img src="docs/assets/brand/synchire-mark.svg" alt="SyncHire 标志：由职业路径、人物与相遇点组成的路标" width="112" />
</p>
...
<p align="center">
  ...
  <a href="#github-pages-byok-体验版"><img alt="AI Provider: BYOK" src="https://img.shields.io/badge/AI%20Provider-BYOK-0F766E" /></a>
</p>
...
访问 [SyncHire Pages](https://rethymus.github.io/SyncHire/) 可直接体验本地优先工作流。此版本是静态站点：不会运行 SyncHire API、模型代理、认证或项目代付账户。面试复盘可在你明确确认后，直接从浏览器向已配置的 OpenAI 兼容、Anthropic 或 Gemini 供应商发请求；是否可用仍取决于供应商的浏览器 CORS 策略。

供应商 Key 仅保存在当前标签页的 `sessionStorage`，不会导出，也不会写入 SyncHire 后端。关闭标签页/窗口通常会清除它；浏览器会话恢复、扩展程序、XSS、被入侵的设备或发布链路不在这一保证范围内。求职数据则默认保存在浏览器本地，便于下次继续，但这不是备份——请在“数据管理”导出 JSON，并可随时分别清除工作区或本标签页密钥。

静态 Pages 不能安全地替代需要受控服务端的功能：图像生成、证件照、GitHub 令牌分析、OAuth、MCP、WebSocket、服务端抓取/上传等会被禁用或给出说明，而不是回退到代理。
...
| GitHub Pages BYOK | 浏览器 | 非敏感工作区数据使用 `localStorage`；供应商 Key 只使用当前标签页的 `sessionStorage` |
...
当前用户视角流程证据已沉淀在中英文 README 截图矩阵中，并由 Playwright 截图回归按需覆盖（默认 e2e 跳过，避免实时时间戳造成截图 diff 噪声）：
`cd frontend && UPDATE_SNAPSHOTS=1 npx playwright test e2e/readme-screenshots.spec.ts --workers=1`。
