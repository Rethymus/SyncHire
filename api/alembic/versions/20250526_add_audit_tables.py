...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('backup_deleted', sa.Boolean(), nullable=False, server_default='false'),
    ...
    op.execute("COMMENT ON TABLE audit_logs IS 'Audit trail for GDPR compliance - tracks all data access and modifications'")
    op.execute("COMMENT ON TABLE data_retention_logs IS 'Data retention and deletion tracking for GDPR Article 7(3)'")
    op.execute("COMMENT ON TABLE consent_logs IS 'User consent tracking for GDPR Article 7 compliance'")
