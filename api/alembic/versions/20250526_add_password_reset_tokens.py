...
def upgrade() -> None:
    ...
    op.create_index('password_reset_tokens_expires_at_idx', 'password_reset_tokens', ['expires_at'])

    # Add comment for table
    op.execute("COMMENT ON TABLE password_reset_tokens IS 'Password reset tokens for user authentication recovery'")
