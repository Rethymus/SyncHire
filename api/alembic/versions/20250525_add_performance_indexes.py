...
def upgrade():
    ...
    op.create_index(
        "ix_resumes_user_created", "resumes", ["user_id", sa.desc("created_at")]
    )

    # Add index on JDs for user filtering
    op.create_index("ix_jds_user_created", "jds", ["user_id", sa.desc("created_at")])
...
def downgrade():
    ...
    op.drop_index(
        "ix_application_status_history_user_id", table_name="application_status_history"
    )
    op.drop_index(
        "ix_application_status_history_application_id",
