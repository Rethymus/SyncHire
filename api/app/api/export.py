...
async def export_applications_csv(
    ...
    try:
        ...
        logger.error(LogCategory.API, f"Error exporting applications: {e}")
        raise HTTPException(
...
async def export_resumes_csv(
    ...
    try:
        ...
        logger.error(LogCategory.API, f"Error exporting resumes: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def export_jds_csv(
    ...
    try:
        ...
        writer.writerow(["ID", "Title", "Company", "Created Date", "Updated Date"])
        ...
        for jd in jds:
            writer.writerow(
                [
                    ...
                    (
                        jd.updated_at.strftime("%Y-%m-%d %H:%M:%S")
                        if jd.updated_at
                        else "N/A"
                    ),
                ]
        ...
        logger.error(LogCategory.API, f"Error exporting job descriptions: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
