...
from app.schemas.schemas_lite import SearchRequest, SearchResponse, MatchRequest, MatchResponse
        ...
        if type in ["all", "resumes"]:
            ...
            for resume in resumes:
                results.append({
                    ...
                    "id": str(resume.id),
                    "title": resume.title,
                    "content": resume.content[:200] + "..." if len(resume.content) > 200 else resume.content,
        ...
        if type in ["all", "jds"]:
            ...
            for jd in jds:
                results.append({
                    ...
                    "content": jd.description[:200] + "..." if len(jd.description) > 200 else jd.description,
...
async def semantic_search(
    ...
    try:
        ...
        logger.error(LogCategory.DATA, f"Semantic search failed: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def match_resume_jd(
    ...
    try:
        ...
        insights = []
        if match_score >= 80:
            insights.append("Strong match - Your profile aligns well with this position")
        elif match_score >= 60:
        ...
        logger.info(LogCategory.AI, f"Match calculated: {request.resume_id} + {request.jd_id} = {match_score}")
        ...
        logger.error(LogCategory.AI, f"Match calculation failed: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_search_suggestions(
    ...
    try:
        ...
        logger.error(LogCategory.DATA, f"Failed to get suggestions: {str(e)}", exc_info=True)
        return {"suggestions": []}
...
async def get_search_statistics(
    ...
    try:
        ...
        logger.error(LogCategory.DATA, f"Failed to get statistics: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
