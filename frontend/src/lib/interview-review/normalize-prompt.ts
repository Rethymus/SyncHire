...
const SOURCE_GUIDE_ZH: Record<InterviewInputSource, string> = {
  recall:
    "输入类型是「面试后回忆」：候选人事后凭记忆写下，语序可能混乱、前后跳跃、甚至遗漏关键问题。请尽量重组为合理的面试顺序，缺失的部分不要凭空补造，归入 unresolved。",
  transcript:
    "输入类型是「会议转写」：通常来自腾讯会议 / 飞书妙记 / Otter 等，已分段，但可能重复、说话人混杂。请去重、标注说话人、保持原意。",
  audio:
    "输入类型是「语音转写」：来自 ASR，常缺少标点、连读、说话人未区分。请补全断句、区分说话人、还原为问答对话。",
};
...
export function buildNormalizePrompt(
  ...
  const zh = `你是一名严谨的面试记录整理员。下面是用户提供的「面试原始记录」，它可能语序混乱、重复或残缺。你的任务是在不改变原意、不杜撰的前提下，把它整理成结构清晰、按合理面试顺序排列的「问答对话」。

${SOURCE_GUIDATE(input.source)}
...
${NORMALIZE_JSON_SCHEMA_HINT}`;
  ...
  const en = `You are a meticulous interview-notes editor. Below is the user's "raw interview record" — it may be out of order, repetitive, or incomplete. Your job is to reorganize it into clearly structured Q&A turns in a sensible interview order, WITHOUT changing meaning or fabricating anything.

${SOURCE_GUIDE_EN(input.source)}
...
// Helper lines kept local so the prompt string reads top-down.
function SOURCE_GUIDATE(s: InterviewInputSource): string {
  return SOURCE_GUIDE_ZH[s];
}
function SOURCE_GUIDE_EN(s: InterviewInputSource): string {
  const map: Record<InterviewInputSource, string> = {
    recall:
      'Input type is "post-interview recall": written from memory afterward, possibly unordered, jumpy, or missing key questions. Reconstruct a sensible order; do NOT fabricate missing parts — put them in unresolved.',
    transcript:
      'Input type is "meeting transcript": usually from Tencent Meeting / Feishu Minutes / Otter, already segmented but possibly duplicated or with mixed speakers. Dedup, tag speakers, preserve meaning.',
    audio:
      'Input type is "ASR transcript": from speech-to-text, often missing punctuation, run-on, speakers unseparated. Add sentence breaks, separate speakers, restore as Q&A dialogue.',
  };
  return map[s];
}
