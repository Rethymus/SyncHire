// Panel-domain copy (bilingual) for the lazily loaded settings panels.
//
// Imported ONLY by ai-provider-panel / capability-panel / discovery-panel
// (and their helpers in panel-shared.tsx). The settings page shell
// (page.tsx, shared.tsx) must NOT import this file — otherwise the whole
// bilingual panel copy + zh catalogs ride along into the settings first load.
//
// Keys rendered by the shell itself (header refresh button, save/reset
// toasts, MetricStrip status labels) live in copy.ts and are merged back in
// by getPanelCopy() so panel JSX can keep a single `copy.*` surface.

import { COPY } from "./copy";
import type { AIProviderId, RuntimeCatalogKind } from "@/lib/ai-runtime-settings";
...
export type PanelCopy = (typeof PANEL_COPY)[keyof typeof PANEL_COPY];

export type SettingsLocale = "en-US" | "zh-CN";

// Panels keep using a single `copy.*` surface: panel-domain groups come from
// PANEL_COPY, while the shell-owned status/actions groups are merged in from
// copy.ts (that module is always loaded by the page shell anyway, so this
// costs nothing extra at runtime).
export function getPanelCopy(locale: SettingsLocale) {
